...
async function originalDimensions(photoId) {
  ...
  const largest = payload.sizes.size.reduce((best, size) => Number(size.width) * Number(size.height) > Number(best.width) * Number(best.height) ? size : best);
...
export default async function handler(request, response) {
  ...
  try {
    ...
    const enrichment = {
      captureId: id, resolvedAt: new Date().toISOString(), status, flickrPhotoId: exact.length === 1 ? exact[0].id : null, sourceFingerprint: source,
      candidateCount: (search.photos.photo || []).length, dimensionMatchCount: dimensionMatches.length, exactMatchCount: exact.length,
